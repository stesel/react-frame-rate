...
export function withReactFrameRate<UpdateProps extends BaseUpdateProps>(
  options: Options<UpdateProps>
) {
  return (
    Component: React.ComponentType<UpdateProps>
  ): React.FC<UpdateProps> => {
    const FrameRate: React.FC<UpdateProps> = (props) => {
      const { updateState, frameRate } = options;

      const { isAnimating } = props;

      const { updateCallback, updateFrameRate, updateAnimation } =
        useFrameRateManager();
